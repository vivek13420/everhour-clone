...
const getUsers = async () => {
  let res = await axios.get(
    `https://cloneofeverhour.herokuapp.com/projects/63356e456e29a1d70aab7c3b`,
  );
  return res;
};

const TeamList = ({ prodata, removePro, handleToggle }) => {
              ...
              {prodata.map(e => {
                return (
                  <Tr>
                    <Td>{e.username}</Td>
                    <Td>
                        {`Project_${Math.floor(Math.random()*10)+1}`}
